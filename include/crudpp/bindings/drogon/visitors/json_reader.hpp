...
struct json_reader
{
    ...
    {
        f.value = json[std::remove_reference_t<decltype(f)>::c_name()].asBool();
    }

    void operator()(crudpp::r_c_n_v auto& f) noexcept
        requires std::same_as<decltype(f.value), uint64_t>
    {
        f.value = json[std::remove_reference_t<decltype(f)>::c_name()].asLargestUInt();
    }
    ...
    {
        f.value = json[std::remove_reference_t<decltype(f)>::c_name()].asUInt();
    }

    void operator()(crudpp::r_c_n_v auto& f) noexcept
        requires std::same_as<decltype(f.value), int64_t>
    {
        f.value = json[std::remove_reference_t<decltype(f)>::c_name()].asLargestInt();
    }
    ...
    {
        f.value = json[std::remove_reference_t<decltype(f)>::c_name()].asInt();
    }

    void operator()(crudpp::r_c_n_v auto& f) noexcept
        requires std::same_as<decltype(f.value), double>
    {
        f.value = json[std::remove_reference_t<decltype(f)>::c_name()].asDouble();
    }
    ...
    {
        f.value = json[std::remove_reference_t<decltype(f)>::c_name()].asFloat();
    }

    void operator()(crudpp::r_c_n_v auto& f) noexcept
        requires std::is_enum_v<decltype(f.value)>
    {
        f.value = decltype(f.value)(json[std::remove_reference_t<decltype(f)>::c_name()].asInt());
    }

    void operator()(crudpp::r_c_n_v auto& f) noexcept
        requires std::same_as<decltype(f.value), std::string>
    {
        f.value = json[std::remove_reference_t<decltype(f)>::c_name()].asString();
    }

    void operator()(crudpp::r_c_n_v auto& f) noexcept
        requires std::convertible_to<decltype(f.value), std::chrono::sys_days>
    {
        f.value = from_drgn_date(json[std::remove_reference_t<decltype(f)>::c_name()].asString());
    }

    void operator()(crudpp::r_c_n_v auto& f) noexcept
        requires std::same_as<decltype(f.value), std::chrono::sys_seconds>
    {
        f.value = from_drgn_time(json[std::remove_reference_t<decltype(f)>::c_name()].asString());
    }

    void operator()(crudpp::r_c_n_v auto& f) noexcept
        requires std::same_as<decltype(f.value), std::chrono::sys_time<std::chrono::milliseconds>>
    {
        f.value = from_drgn_time_ms(json[std::remove_reference_t<decltype(f)>::c_name()].asString());
    }
