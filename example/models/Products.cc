...
Skus Products::getSKU(const drogon::orm::DbClientPtr &clientPtr) const {
    std::shared_ptr<std::promise<Skus>> pro(new std::promise<Skus>);
    std::future<Skus> f = pro->get_future();
    getSKU(clientPtr, [&pro] (Skus result) {
        try {
...
{
    const static std::string sql = "select * from skus where product_id = ?";
    *clientPtr << sql
               << *id_
...
std::vector<Reviews> Products::getReviews(const drogon::orm::DbClientPtr &clientPtr) const {
    std::shared_ptr<std::promise<std::vector<Reviews>>> pro(new std::promise<std::vector<Reviews>>);
    std::future<std::vector<Reviews>> f = pro->get_future();
    getReviews(clientPtr, [&pro] (std::vector<Reviews> result) {
        try {
            pro->set_value(result);
        }
        catch (...) {
...
void Products::getReviews(const DbClientPtr &clientPtr,
                          ...
                          const ExceptionCallback &ecb) const
{
    const static std::string sql = "select * from reviews where product_id = ?";
    *clientPtr << sql
               << *id_
...
std::vector<std::pair<Carts,CartsProducts>> Products::getCarts(const drogon::orm::DbClientPtr &clientPtr) const {
    std::shared_ptr<std::promise<std::vector<std::pair<Carts,CartsProducts>>>> pro(new std::promise<std::vector<std::pair<Carts,CartsProducts>>>);
    std::future<std::vector<std::pair<Carts,CartsProducts>>> f = pro->get_future();
    getCarts(clientPtr, [&pro] (std::vector<std::pair<Carts,CartsProducts>> result) {
        try {
            pro->set_value(result);
        }
        catch (...) {
...
void Products::getCarts(const DbClientPtr &clientPtr,
                        ...
                        const ExceptionCallback &ecb) const
{
    const static std::string sql = "select * from carts,carts_products where carts_products.product_id = ? and carts_products.cart_id = carts.id";
    *clientPtr << sql
               << *id_
               >> [rcb = std::move(rcb)](const Result &r){
